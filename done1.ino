...
// Ganti dengan IP komputer server Anda
const String serverIP = "khaki-turtle-323390.hostingersite.com";
const String projectFolder = "SmartHomeDC";  // Folder proyek dalam htdocs
...
//======================================== 
// VOLTAGE COMPENSATION - BARU
//======================================== 
struct BatteryCompensation {
    float internalResistance = 0.06;  // 50 milli-ohm (adjust sesuai baterai)
...
struct BatteryMonitoring {
    ...
    float recoverySOC = 38.0; // Recovery pada 38% (hysteresis)
...
// Global instance - TAMBAH INI JUGA
...
const float VOLTAGE_SOC_TABLE[][2] = {
  {12.860, 100.0}, {12.654, 90.0}, {12.448, 80.0}, 
  {12.242, 70.0}, {12.036, 60.0}, {11.830, 50.0}, 
  {11.624, 40.0}, {11.512, 35.0}, {11.418, 30.0}, 
  {11.212, 20.0}, {11.006, 10.0}, {10.800, 0.0} 
};
...
void initializeINA219Battery() {
    ...
    if (error == 0) {
        ...
        Serial.println("⚠️  NOTE: Address 0x40 might conflict with TCA sensors");
...
void configureINA219Battery() {
    ...
    // Konfigurasi sederhana untuk pembacaan voltage baterai saja
    // BRNG=1 untuk 32V range (cocok untuk baterai 12V)
    uint16_t config = 0;
    ...
    // Tulis konfigurasi (tidak perlu kalibrasi untuk voltage saja)
    ...
    Serial.printf("- Current/Power: DISABLED (not needed)\n");
...
void configureINA219(INASensor* sensor) {
  ...
  // Konfigurasi khusus untuk USB (3A max)
    ...
    float shuntResistor = 0.1;           // Nilai shunt resistor (ohm) - sesuaikan!
...
//======================================== 
// DEVICE CONTROL FUNCTIONS - DENGAN MODE CONTROL (TIDAK DIUBAH)
//======================================== 
void setDevice(String deviceId, bool state) {
    // BATTERY SAFETY CHECK - INI YANG PENTING!
    ...
    // Execute device control (EXISTING CODE - JANGAN DIUBAH)
    if (deviceId == "L1") {
...
//======================================== 
// SENSOR READING FUNCTIONS - UPDATE UNTUK BATTERY SENSOR
//======================================== 
void readSensors() {
  Serial.println();
  ...
  readPIRSensors();
   ...
   //Baca tegangan baterai dari INA219 (bukan GPIO34)
...
void readPowerData() {
    ...
    systemState.totalPower = systemState.powerL1.power + systemState.powerL2.power + 
                             systemState.powerL3.power + systemState.powerL4.power + 
                             systemState.powerL5.power + systemState.powerK1.power + 
                             systemState.powerK2.power + systemState.powerUSB.power;
                             
    systemState.totalCurrent = systemState.powerL1.current + systemState.powerL2.current + 
                               systemState.powerL3.current + systemState.powerL4.current + 
                               systemState.powerL5.current + systemState.powerK1.current + 
                               systemState.powerK2.current + systemState.powerUSB.current;
}
  ...
  // Interpolate for voltages in between
    ...
    for (int i = 0; i < tableSize - 1; i++) {
      if (voltage >= VOLTAGE_SOC_TABLE[i+1][0] && voltage < VOLTAGE_SOC_TABLE[i][0]) {
        ...
        systemState.battery.soc = soc_lower + 
          ((voltage - v_lower) * (soc_upper - soc_lower)) / (v_upper - v_lower);
        break;
      }
    }
  }
...
void calculateEstimatedRuntime() {
  if (systemState.totalPower > 0) {
    float batteryCapacity = 200.0; // 200Ah Kapasitas Baterai 
    float remainingCapacity = batteryCapacity * (systemState.battery.soc / 100.0);
    systemState.battery.estimatedRuntime = (remainingCapacity * systemState.battery.voltage) / systemState.totalPower;
  } else {
...
void executeAutoMode() {
    ...
    // PERBAIKAN: Matikan semua device dulu, baru tentukan yang boleh hidup
    // Implementasi Tabel 3.3 Skenario Rekomendasi Beban Prioritas
    ...
    if (soc >= 90.0) {
        ...
        setDevice("L3", false);  // ← TAMBAHKAN INI!
        setDevice("L4", false);
        setDevice("L5", false);
        setDevice("K1", false);
        setDevice("K2", false);
...
void executePriorityDevicesLogic() {
    ...
    executeRoomControlLogicWithSOCLimit(0.0); // K2 sudah dimatikan sebelumnya
}

void executeLampsOnlyLogic() {
    // Hanya lampu yang bisa aktif berdasarkan sensor
    executeRoomControlLogicWithSOCLimit(0.0); // Kipas sudah dimatikan sebelumnya
}
...
void executeRoomControlLogicWithSOCLimit(float minSOCRequired) {
    ...
    if (currentSOC >= minSOCRequired && currentSOC >= 35.0) {
        ...
        if (systemState.sensors.lightLevel < LIGHT_THRESHOLD && systemState.sensors.pirTeras) {
            if (!systemState.devices.L1) {
                ...
                // PERBAIKAN: Reset timer jika ada gerakan baru saat lampu sudah nyala
    ...
    // KIPAS - dengan ENHANCED SMART TIMER (hanya jika SOC ≥ 80%)
    if (currentSOC >= 80.0 && currentSOC >= minSOCRequired) {
        ...
        // Kipas Ruang Tamu (K2)
        if (systemState.sensors.pirRuangTamu && systemState.sensors.temperatureRTamu > TEMP_THRESHOLD) {
            if (!systemState.devices.K2) {
                setDevice("K2", true);
                systemState.deviceTimers[2] = currentTime;
                Serial.println("✅ Auto: Kipas Ruang Tamu ON (motion & temp > 32°C & SOC ≥ 80%)");
            } else {
                // Reset timer jika ada gerakan baru DAN suhu masih tinggi
                unsigned long oldTimer = systemState.deviceTimers[2];
                systemState.deviceTimers[2] = currentTime;
                Serial.println("🔄 Auto: Kipas Ruang Tamu timer RESET (motion & temp still high)");
                Serial.printf("🔄 Timer K2 RESET: %lu → %lu\n", oldTimer, currentTime);
            }
        }
        
        // Kipas Kamar (K1)
        if (systemState.sensors.pirKamar && systemState.sensors.temperatureKamar > TEMP_THRESHOLD) {
            if (!systemState.devices.K1) {
                ...
                systemState.deviceTimers[4] = currentTime;
                Serial.println("🔄 Auto: Kipas Kamar timer RESET (motion & temp still high)");
                Serial.printf("🔄 Timer K1 RESET: %lu → %lu\n", oldTimer, currentTime);
            }
        }
    } else if (currentSOC < 80.0) {
        // Matikan kipas jika SOC < 80%
        if (systemState.devices.K1) {
            setDevice("K1", false);
        ...
        if (systemState.devices.K2) {
            setDevice("K2", false);
            Serial.println("⚠️ Auto: Kipas Ruang Tamu OFF (SOC < 80%)");
        }
    }
...
void initializeWiFi() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        if(connecting_process_timed_out == 0) {
            ...
            delay(2000);
            WiFi.begin(ssid, password);
            connecting_process_timed_out = 40; // Reset timer, kasih waktu lebih lama
            }
    }
...
bool detectCharging() {
    ...
    // Hitung trend voltage (rata-rata 3 sample terakhir)
...
void handleBatteryEmergency() {
    ...
    if (batteryMonitor.emergencyModeActive) {
        ...
        else if (batteryMonitor.isCharging && 
                 systemState.battery.voltage > (batteryMonitor.preEmergencyVoltage + 0.2)) {
            canRecover = true;
...
//======================================== 
// SETUP & MAIN LOOP - UPDATE UNTUK BATTERY SENSOR
//======================================== 
void setup() {
    Serial.begin(115200);
    ...
    // TAMBAHAN: Debug initial battery voltage reading
...
void loop() {
    ...
    if(WiFi.status() == WL_CONNECTED) {
        ...
        // STEP B: Battery monitoring - INI YANG BARU!
        detectCharging();
